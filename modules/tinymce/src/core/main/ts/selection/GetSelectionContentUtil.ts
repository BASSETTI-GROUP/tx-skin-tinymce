...
const toLeaf = (node: Node, offset: number) =>
  Traverse.leaf(SugarElement.fromDom(node), offset);

const walk = (dom: DOMUtils, walkerFn: Walker, startNode: Node, callbacks: WalkerCallbacks, endNode?: Node, skipStart: boolean = true) => {
  ...
  const collectCallbacks = {
    ...
    cef: (node) => {
      ...
      return Arr.sort(sections, (a, b) =>
        (SandNode.documentPositionPreceding(a.elements[0].dom, b.elements[0].dom)) ? 1 : -1);
    }
  };

  return collect(dom, rng.commonAncestorContainer, startNode, endNode, collectCallbacks, false);
};
...
const fromRange = (dom: DOMUtils, range: Range): string => {
  if (range.collapsed) {
    return '';
  } else {
